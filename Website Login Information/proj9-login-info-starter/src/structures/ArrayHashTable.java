...
public class ArrayHashTable<K, V> implements HashTable<K, V> {
  ...
  public ArrayHashTable(CollisionHandler <K> collisionHdler) {
      ...
      this.keyArray = (K[]) new Object[capacity];
      this.valueArray = (V[]) new Object[capacity];
      this.isActiveArray = new boolean[capacity];
  }
  ...
  private void resizeArray() {
   //TODO: Implement this method
   capacity = capacity * 2;
   K[] oldKeyArray = keyArray;
    this.keyArray = (K[]) new Object[capacity];

    V[] oldValueArray = valueArray;
    this.valueArray = (V[]) new Object[capacity];

    boolean[] oldIsActive = isActiveArray;
    this.isActiveArray = new boolean[capacity];

    for(int i = 0; i < oldKeyArray.length; i++){
      if(oldIsActive[i] == true){
        int hash = getHashOfKey(oldKeyArray[i]);
        int index = collisionHandler.probe(hash, isActiveArray, capacity);
        keyArray[index] = oldKeyArray[i];
        valueArray[index] = oldValueArray[i];
  ...
  */
  ...
  public void put(K key, V value) {
    ...
    if(getIndex(hash,key) >= 0){
      valueArray[index] = value;
    } else{
      index = resolveCollision(hash);
      isActiveArray[index] = true;
      keyArray[index] = key;
      valueArray[index] = value;
      count++;
  ...
  public V getValue(K target) {
   //TODO: Implement this method
    int hash = getHashOfKey(target);
    int index = getIndex(hash, target);
    if(index >= 0){
  ...
  public V remove(K targetKey)throws ElementNotFoundException {
   //TODO: Implement this method
    int hash = getHashOfKey(targetKey);
    int index = getIndex(hash,targetKey);
    if(index == -1){
      throw new ElementNotFoundException("table");
    }
    else{
